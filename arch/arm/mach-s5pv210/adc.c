...
unsigned int s3c_adc_convert(void)
{
	...
	unsigned long data1;
       ...
       //printk(" adc.c:s3c_adc_convert ---adc_port= %d \n",adc_port);
...
static int __init s3c_adc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
       ...
       #if 1
	//denis_wei add for test 2010-11-23
