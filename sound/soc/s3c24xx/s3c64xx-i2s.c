...
#if 0//
#define pr_debug(x...) printk("[s3c64xx-i2s]: "x)
...
static int s5p_i2s_var_clear(void)
{
	pr_debug("enter s5p_i2s_var_clear();\n");
	tx_clk_enabled = 0;
	rx_clk_enabled = 0;
	audio_clk_gated = 0; /* At first, clock & i2s0_pd is enabled in probe() */
	suspended_by_pm = 0;
}
//EXPORT_SYMBOL(s5p_i2s_var_clear);


static int s5p_i2s_checkAvr(void)
{
	pr_debug("s5p_i2s_checkAvr tx_clk_enabled=%d, rx_clk_enabled=%d, audio_clk_gated=%d, suspended_by_pm=%d.\n"
		, tx_clk_enabled 
		, rx_clk_enabled 
		, audio_clk_gated
		, suspended_by_pm
...
void s5p_i2s_set_clk_enabled(struct snd_soc_dai *dai, bool state)
{
	...
	if (i2s->sclk_audio == NULL) {
		pr_debug("already audio sclk_audio is NULL! \n");
		return;
	}
...
void s5p_i2s_do_suspend(struct snd_soc_dai *dai)
{
	...
	pr_debug("enter %s(); i2s->regs=0x%08x\n", __func__, i2s->regs);
	ur_debug("i2s->suspend_iiscon=%08x\n", i2s->suspend_iiscon);
	ur_debug("i2s->suspend_iispsr=%08x\n", i2s->suspend_iispsr);
	ur_debug("i2s->suspend_iisahb=%08x\n", i2s->suspend_iisahb);
	ur_debug("dai->id=%08x, audio_clk_gated===%d\n", dai->id, audio_clk_gated);
...
void s5p_i2s_do_resume(struct snd_soc_dai *dai)
{
	...
	if(first_enter_flg && dai->id==0) {
		ur_debug("===========first enter func================\n");
		s5p_i2s_var_clear();
		first_enter_flg = 0;
		//s5p_i2s_set_audio_clk_gate(0);
	}
	...
	if(dai->id==0) {
		...
		u32 val;
		u32 reg;
		printk("s5p_i2s_do_resume, set clk gate, dai->id: %d\n", dai->id);
	...
	#if 0
	pr_debug("enter %s(); i2s->regs=0x%08x\n", __func__, i2s->regs);
	ur_debug("i2s->suspend_iismod=%08x\n", i2s->suspend_iismod);
	ur_debug("i2s->suspend_iiscon=%08x\n", i2s->suspend_iiscon);
	ur_debug("i2s->suspend_iispsr=%08x\n", i2s->suspend_iispsr);
	ur_debug("i2s->suspend_iisahb=%08x\n", i2s->suspend_iisahb);
	ur_debug("dai->id=%08x\n", dai->id);
	...
	if (audio_clk_gated) {		/* Clk/Pwr is gated? */
		...
		i2s->suspend_iiscon |= 0x80000000;	//urbetter
		...
		printk("Resume and registers restored.\n");
				  ...
				  int div_id, int div)
{
	...
	pr_debug("s5p_i2s_set_clkdiv();   i2s->regs=x%08x\n", i2s->regs);
...
static void s3c64xx_iis_dai_init(struct snd_soc_dai *dai)
{
	...
	dai->playback.channels_min = 2;	/* I2S dai : match Playback channels_min value with wm8580_dai(@wm8580.c) */
