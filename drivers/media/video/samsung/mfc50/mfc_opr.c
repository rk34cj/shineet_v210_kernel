...
static void mfc_set_dec_stream_buffer(mfc_inst_ctx *mfc_ctx, int buf_addr, unsigned int buf_size)
{
	...
	dmac_clean_range(in_vir, in_vir + ALIGN_TO_32B(buf_size));
#endif
...
static MFC_ERROR_CODE mfc_encode_header(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	dmac_inv_range(in_vir, in_vir + init_arg->out_header_size);
#endif
...
static MFC_ERROR_CODE mfc_encode_one_frame(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	dmac_clean_range(in_vir, in_vir + ALIGN_TO_128B(mfc_ctx->img_width)*ALIGN_TO_32B(mfc_ctx->img_height));
	in_vir = phys_to_virt(enc_arg->in_CbCr_addr);
	dmac_clean_range(in_vir, in_vir + ALIGN_TO_128B(mfc_ctx->img_width)*ALIGN_TO_32B(mfc_ctx->img_height/2));
#endif
	...
	dmac_inv_range(in_vir, in_vir + enc_arg->out_encoded_size);
#endif
...
//                dec_arg->out_crop_top_offset, dec_arg->out_crop_bottom_offset, dec_arg->out_crop_left_offset, dec_arg->out_crop_right_offset);
	...
	if (((READL(MFC_SI_DISPLAY_STATUS) & 0x3) == DECODING_DISPLAY) ||
		...
		dmac_inv_range(out_Y_vir, out_Y_vir + ALIGN_TO_128B(mfc_ctx->widthFIMV1)*ALIGN_TO_32B(mfc_ctx->heightFIMV1));
		out_C_vir = phys_to_virt(dec_arg->out_display_C_addr);
		dmac_inv_range(out_C_vir, out_C_vir + ALIGN_TO_128B(mfc_ctx->widthFIMV1)*ALIGN_TO_32B(mfc_ctx->heightFIMV1)/2);
	}
#endif
