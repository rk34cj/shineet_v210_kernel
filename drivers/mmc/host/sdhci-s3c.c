...
static struct sdhci_host * s_s3c_host[MAX_BUS_CLK] = {NULL, NULL, NULL, NULL}; //urbetter+
...
//urbetter+
void sdhci_s3c_refresh_card(int busId)
{
	struct sdhci_s3c *sc;

	if(busId < 0 || busId > MAX_BUS_CLK || !s_s3c_host[busId])
		return;

	sc = sdhci_priv(s_s3c_host[busId]);
	tasklet_schedule(&sc->host->card_tasklet);
}
EXPORT_SYMBOL(sdhci_s3c_refresh_card);
//urbetter+ end
